...
Linux.UseAcrylic = true
Linux.AcrylicEnabled = true
Linux.Themes = {
...
BlurFolder.Name = "LinuxAcrylicBlur"

local function createAcrylic()
    local Part = Instance.new("Part")
    Part.Name = "LinuxAcrylicBody"
...
local AcrylicBlur = function()
    ...
    return function(distance)
        ...
        local comp = Linux.Instance("Frame", {
...
local AcrylicPaint = function()
    local AcrylicBlurInstance = AcrylicBlur()
    
    return function(props)
        local AcrylicPaint = {}
        
        AcrylicPaint.Frame = Linux.Instance("Frame", {
            Size = UDim2.fromScale(1, 1),
        ...
        local ShadowFrame = Linux.Instance("ImageLabel", {
            ...
            Parent = AcrylicPaint.Frame
        })
        
        local CornerFrame = Linux.Instance("UICorner", {
            CornerRadius = UDim.new(0, 8),
            Parent = AcrylicPaint.Frame
        })
        
        local BackgroundFrame = Linux.Instance("Frame", {
        ...
        local BackgroundCorner = Linux.Instance("UICorner", {
            CornerRadius = UDim.new(0, 8),
            Parent = BackgroundFrame
        })
        
        local GradientFrame = Linux.Instance("Frame", {
            BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        ...
        local GradientCorner = Linux.Instance("UICorner", {
            CornerRadius = UDim.new(0, 8),
        ...
        local Gradient = Linux.Instance("UIGradient", {
            Rotation = 90,
            Color = ColorSequence.new(Color3.fromRGB(40, 40, 40), Color3.fromRGB(40, 40, 40)),
            Parent = GradientFrame
        })
        
        local NoiseTexture1 = Linux.Instance("ImageLabel", {
        ...
        local NoiseCorner1 = Linux.Instance("UICorner", {
            CornerRadius = UDim.new(0, 8),
            Parent = NoiseTexture1
        })
        
        local NoiseTexture2 = Linux.Instance("ImageLabel", {
        ...
        local NoiseCorner2 = Linux.Instance("UICorner", {
            CornerRadius = UDim.new(0, 8),
            Parent = NoiseTexture2
        })
        
        local BorderFrame = Linux.Instance("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.fromScale(1, 1),
            ZIndex = 2,
            Parent = AcrylicPaint.Frame
        ...
        local BorderCorner = Linux.Instance("UICorner", {
            CornerRadius = UDim.new(0, 8),
            Parent = BorderFrame
        })
        
        local BorderStroke = Linux.Instance("UIStroke", {
            Transparency = 0.5,
            Thickness = 1,
        ...
        if Linux.UseAcrylic then
            Blur = AcrylicBlurInstance()
...
function Linux.SaveThemeSettings()
    local success, result = pcall(function()
        local themeData = {
            ...
            UseAcrylic = Linux.UseAcrylic,
            AcrylicEnabled = Linux.AcrylicEnabled
        }
...
function Linux.LoadThemeSettings()
    local success, result = pcall(function()
        if readfile and isfile(Linux.ConfigFolder .. "/theme.json") then
            ...
            Linux.UseAcrylic = themeData.UseAcrylic or false
            Linux.AcrylicEnabled = themeData.AcrylicEnabled or false
            return true
        else
...
function Linux:ToggleAcrylic(value)
    Linux.AcrylicEnabled = value
    if Linux.UseAcrylic then
        if value then
            Acrylic.Enable()
        else
...
function Linux.Create(config)
    ...
    if Linux.UseAcrylic then
        local AcrylicPaintInstance = Acrylic.AcrylicPaint()
        AcrylicPaintInstance.Frame.Parent = Main
        AcrylicPaintInstance.Frame.ZIndex = 0
        Main.BackgroundTransparency = 1
        Main.BorderSizePixel = 0
        
        if AcrylicPaintInstance.AddParent then
            AcrylicPaintInstance.AddParent(Main)
        end
        
        Linux.AcrylicPaintInstance = AcrylicPaintInstance
    else
        Linux.Instance("UICorner", {
            Parent = Main,
            CornerRadius = UDim.new(0, 8)
    ...
    CloseButton.MouseButton1Click:Connect(function()
        LinuxUI:Destroy()
        ToggleUI:Destroy()
        if Linux.UseAcrylic and Linux.AcrylicPaintInstance and Linux.AcrylicPaintInstance.Model then
            Linux.AcrylicPaintInstance.Model:Destroy()
        end
    end)
    ...
    if config.ConfigSave ~= false then
        ...
        if Linux.UseAcrylic then
            SettingsTab.Toggle({
                ...
                Default = Linux.AcrylicEnabled,
                Callback = function(state)
                    Linux:ToggleAcrylic(state)
                end
            })
        end
